...
try:
    ...
    print(
        f"Error: {workflow_file} not found. Will create a new one based on the plan."
    )
        ...
        "jobs": {
            "deploy": {
                ...
                "steps": [
                    ...
                    },  # Assuming requirements-docs.txt is the target
...
if "jobs" in data and "deploy" in data["jobs"]:
    if data["jobs"]["deploy"].get("permissions", {}).get("contents") != "write":
        if (
            "permissions" not in data["jobs"]["deploy"]
            or data["jobs"]["deploy"]["permissions"] is None
        ):
    ...
    print(
        "Warning: 'jobs.deploy' structure not found or incomplete. Rebuilding deploy job structure."
    )
    data["jobs"] = data.get("jobs", {})
    data["jobs"]["deploy"] = data["jobs"].get(
        "deploy", {}
    )  # Ensure deploy key exists
    if not isinstance(data["jobs"]["deploy"], dict):
        data["jobs"]["deploy"] = {}  # Ensure deploy value is a dict
    data["jobs"]["deploy"]["permissions"] = {"contents": "write"}
    # Add other necessary fields if they are missing, based on a standard template
    if "runs-on" not in data["jobs"]["deploy"]:
        data["jobs"]["deploy"]["runs-on"] = "ubuntu-latest"
    if "steps" not in data["jobs"]["deploy"]:
        data["jobs"]["deploy"]["steps"] = [  # Add default steps if missing
            {"name": "Checkout code", "uses": "actions/checkout@v4"},
            {
                "name": "Set up Python",
                "uses": "actions/setup-python@v5",
...
if not isinstance(data.get("on"), dict):
    data["on"] = {}
if not isinstance(data["on"].get("push"), dict):
    data["on"]["push"] = {}
...
if (
    "jobs" in data
    and isinstance(data["jobs"], dict)
    and "deploy" in data["jobs"]
    and isinstance(data["jobs"]["deploy"], dict)
    and "steps" in data["jobs"]["deploy"]
    and isinstance(data["jobs"]["deploy"]["steps"], list)
):
    for step in data["jobs"]["deploy"]["steps"]:
        if (
            isinstance(step, dict)
            and step.get("uses")
            and "peaceiris/actions-gh-pages" in step.get("uses", "")
        ):
            deploy_step_found = True
            if (
                "with" not in step
                or step["with"] is None
                or not isinstance(step["with"], dict)
            ):  # Ensure 'with' key exists and is a dict
                step["with"] = {}
                changes_made = True
            ...
            if step["with"].get("user_name") != "github-actions[bot]":
                ...
                changes_made = True
            if (
                step["with"].get("user_email")
                != "github-actions[bot]@users.noreply.github.com"
            ):
                step["with"]["user_email"] = (
                    "github-actions[bot]@users.noreply.github.com"
                )
                changes_made = True
            if (
                step["with"].get("publish_dir") != "./site"
            ):  # Ensure publish_dir is there and correct
    ...
    print(
        "Warning: 'jobs.deploy.steps' structure not found or invalid. Cannot configure deployment step effectively."
    )
...
if changes_made:
    with open(workflow_file, "w") as f:
        yaml.dump(
            data, f, sort_keys=False, default_flow_style=False, width=1000
        )  # Added flow and width for better formatting
