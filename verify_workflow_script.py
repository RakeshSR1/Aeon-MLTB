...
if "jobs" in data and "deploy" in data["jobs"]:
    ...
    print(
        "Warning: 'jobs.deploy' structure not found in workflow file. Cannot set permissions."
    )
...
if (
    "on" in data
    and isinstance(data["on"], dict)
    and "push" in data["on"]
    and isinstance(data["on"]["push"], dict)
):
    if data["on"]["push"].get("branches") != ["**"]:
        data["on"]["push"]["branches"] = ["**"]
        changes_made = True
elif "on" in data and isinstance(
    data["on"], dict
):  # if 'on' exists but not 'push' or 'push' is not a dict
