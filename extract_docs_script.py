...
with open("bot/helper/ext_utils/help_messages.py", "r") as f:
    content = f.read()

# Helper to extract string variable values
def extract_variable_value(var_name, source_content):
    match = re.search(f"^{var_name}\s*=\s*\"\"\"(.*?)\"\"\"", source_content, re.DOTALL | re.MULTILINE)
    if match:
...
def extract_dict_value(dict_name, source_content):
    ...
    match = re.search(f"^{dict_name}\s*=\s*{{(.*?)}}", source_content, re.DOTALL | re.MULTILINE)
    if not match:
        return None
    ...
    if dict_name == "YT_HELP_DICT":
        ...
        yt_dict_references = {
            "main": "yt", "New-Name": "new_name", "Zip": "zip_arg", "Quality": "qual",
            "Options": "yt_opt", "Multi-Link": "multi_link", "Same-Directory": "same_dir",
            "Thumb": "thumb", "Split-Size": "split_size", "Upload-Destination": "upload",
            "Rclone-Flags": "rcf", "Bulk": "bulk", "Sample-Video": "sample_video",
            "Screenshot": "screenshot", "Convert-Media": "convert_media", "Force-Start": "force_start",
            "Name-Substitute": "name_sub", "TG-Transmission": "transmission",
            "Thumb-Layout": "thumbnail_layout", "Leech-Type": "leech_as", "FFmpeg-Cmds": "ffmpeg_cmds"
        }
        for key, var_name_or_fstring in yt_dict_references.items():
            ...
            if var_name_or_fstring == "new_name" and key == "New-Name": # Handle specific f-string case
        ...
        mirror_dict_references = {
            "main": "mirror", "New-Name": "new_name", "DL-Auth": None, # None for direct string
            "Headers": None, "Extract/Zip": "extract_zip",
            "Select-Files": None, "Torrent-Seed": "seed", "Multi-Link": "multi_link",
            "Same-Directory": "same_dir", "Thumb": "thumb", "Split-Size": "split_size",
            "Upload-Destination": "upload", "Rclone-Flags": "rcf", "Bulk": "bulk",
            "Join": "join", "Rclone-DL": "rlone_dl", "Tg-Links": "tg_links",
            "Sample-Video": "sample_video", "Screenshot": "screenshot",
            "Convert-Media": "convert_media", "Force-Start": "force_start",
            "User-Download": "user_download", "Name-Substitute": "name_sub",
            "TG-Transmission": "transmission", "Thumb-Layout": "thumbnail_layout",
            "Leech-Type": "leech_as", "FFmpeg-Cmds": "ffmpeg_cmds"
        }
        ...
        parsed_dict["DL-Auth"] = "<b>Direct link authorization</b>: -au -ap\n\n/cmd link -au username -ap password" # Manual
        parsed_dict["Headers"] = "<b>Direct link custom headers</b>: -h\n\n/cmd link -h key:value|key1:value1" # Manual
        parsed_dict["Extract/Zip"] = extract_variable_value("extract_zip", source_content)
        parsed_dict["Select-Files"] = "<b>Bittorrent/JDownloader/Sabnzbd File Selection</b>: -s\n\n/cmd link -s or by replying to file/link" # Manual
        ...
        for key, var_name in mirror_dict_references.items():
            if var_name: # If it's a variable name
                ...
                parsed_dict[key] = f"Content for {key} (direct string in dict) needs manual extraction or a more complex parser."
...
markdown_output += "This page details the various command-line options available for tasks.\n\n"

# 2. General Options
general_options_vars = [
    ("New Name", "new_name"), ("Multi-Link", "multi_link"), ("Same Directory", "same_dir"),
    ("Thumbnail", "thumb"), ("Split Size", "split_size"), ("Upload Destination", "upload"),
    ("Rclone Flags", "rcf"), ("Extract/Zip", "extract_zip"), ("Join Splitted Files", "join"),
    ("Sample Video", "sample_video"), ("Screenshots", "screenshot"), ("Bittorrent Seed", "seed"),
    ("Quality Buttons (YT-DLP)", "qual"), #("YT-DLP Options", "yt_opt"), # yt_opt is handled below
    ("Convert Media", "convert_media"), ("Force Start", "force_start"), ("Name Substitution", "name_sub"),
    ("TG Transmission", "transmission"), ("Thumbnail Layout", "thumbnail_layout"),
    ("Leech As Document/Media", "leech_as"), ("FFmpeg Commands", "ffmpeg_cmds")
]
...
for title, var_name in general_options_vars:
    ...
    if value:
        ...
        value_md = value.replace("<br>", "\n").replace("<b>", "**").replace("</b>", "**").replace("<code>", "`").replace("</code>", "`")
        value_md = re.sub('<a href=["\'](.*?)["\']>(.*?)</a>', r'[\2](\1)', value_md) # Convert <a> tags
